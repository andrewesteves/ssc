 ...
 * @author Andrew
 */
public class ClienteMensagem extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(usuarioAtual != null && usuarioAtual.getAcesso() == Acesso.CLIENTE) {
            try{
                ...
                a.setId(Integer.parseInt(id_atendimento));
                Atendimento atendimento = fachada.selecionar(a);                
                
                if(acao != null && acao.equals("criar")) {
                    Mensagem m = new Mensagem();
                    m.setAtendimento(atendimento);
                    m.setDescricao(request.getParameter("descricao"));
                    m.setUsuario(usuarioAtual);
                    fachada.criar(m);
                    atendimento = fachada.selecionar(a);
                    request.setAttribute("success", "Mensagem enviada com sucesso.");
                }
                
                request.setAttribute("atendimento", atendimento);
                request.getRequestDispatcher("/cliente_mensagem.jsp").forward(request, response);
            }catch(Exception e) {
